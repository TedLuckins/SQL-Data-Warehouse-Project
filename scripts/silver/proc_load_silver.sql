...
Script Purpose:
  This stored procedure cleans and loads data to the 'silver schema from the 'bronze' schema .
  This procedure first, truncate the silver tables.
  Secondly, Uses INSERT command to load clean data from corresponding bronze tables.
================================================================================================
Parameters:
  None
	...
	BEGIN TRY
		...
		SELECT
			...
			CASE WHEN UPPER(TRIM(cst_material_status)) = 'M' THEN 'Married'
				...
				ELSE 'n/a' -- Normalise marital status to readable format
		...
		SELECT
			...
			CAST(
				...
				AS DATE
			) AS prd_start_dt -- Calculate end date as one day before the next start date
		...
		SELECT
			...
			CASE WHEN TRIM(UPPER(gen)) IN ('M', 'MALE') THEN 'Male'
				WHEN TRIM(UPPER(gen)) IN ('F', 'FEMALE') THEN 'Fermale'
				ELSE 'n/a'
			END AS gen
